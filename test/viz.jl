...
meshviz(mesh::MeshTools.Mesh; args...)  = meshviz(mesh.V, mesh.F; args...)

function meshviz(meshes::Vector; shift_coordinates=false, args...) 
    N = length(meshes)
    resolution = (700*N, 400* N)
    fig = Figure(resolution=resolution)
    for i=1:N
...
function viz_field!(mesh::MeshTools.Mesh,field=nothing, field_type=:face; kwargs...)
    if field===nothing
        field = mesh.normals
    end
...
function viz_point!(mesh::MeshTools.Mesh, id::Int; kwargs...)
    xyz = mesh.V[:,id]
    scatter!(xyz...,; kwargs...)
end
